 ...
 * <em>inconsistent</em> ones.  In a consistent block, lines are broken
 ...
 * by opening a block which sets the indentation level to the column 
 * where the <code>G</code> ends up and outdenting the lines with 
 * the <code>and</code>:
 * <pre>
 ...
 * <code>beginC(ind)</code>, where <code>ind</code> is the default
 ...
 *
 * <p>The backend might throw exceptions of the type indicated by the
 * type parameter {@code Exc}.
 * Such exceptions get passed through to the caller of the
 * Layouter.  Note that since text usually gets buffered before it is
 * sent to the backend, exceptions thrown by calls to Layouter methods 
 * might not be caused directly by that method call but by an earlier 
 * one getting forwarded to the backend.
 *
 * <p>The algorithm used is essentially the classical one from
 * Derek C. Oppen: <i>Prettyprinting</i>, TOPLAS volume 2 number 4, 
 * ACM, 1980, pp. 465-483, with some minor extensions.  It has the
 * property that if the input contains enough actual text, i.e.
 * not just arbitrarily long sequences of calls to <code>beginC</code>,
 * <code>mark</code>, etc., then pretty-printing uses constant
 * space, and time linear in the size of the input.  In fact,
 * output will begin before the whole input has been given, so
 * this class can be used to pretty-print a stream of
 * data.
 *
 ...
 * portions of blocks between breaks if they are to be printed in a
 * single line.  The actual laying-out, that is choosing whether to
 * break lines or not is done by a Printer object, which in turn sends
...
public class Layouter<Exc extends Exception> {
	...
	= new java.util.LinkedList<StreamToken>();
	 ...
	 */
	public static Layouter<IOException> 
	 ...
	 */
	public static Layouter<IOException> 
	getWriterLayouter(java.io.Writer writer,int lineWidth) {
	 ...
	 */
	public static Layouter<IOException> 
	getWriterLayouter(java.io.Writer writer,int lineWidth,int indentation) {
	...
	private abstract class StreamToken {
		...
		/** Returns whether this is a <code>BreakToken</code>.
		 * It returns <code>false</code>, and is overriden by 
		 * <code>BreakToken</code> to return true. */
